...
public class TransactionCount {
	...
	public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException {
			...
			output.collect(new Text(transactiontokens[2]),one);
		...
		public void reduce(Text key, Iterator<IntWritable> values, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException { //{little: {1,1}} 
			...
			while(values.hasNext()) {
				transactionCount += values.next().get();
				
			}
			output.collect(key, new IntWritable(transactionCount));
		}
		}
	...
	public static void main(String[] args) throws Exception {
		...
		conf.setJobName("counting the number of transaction");
